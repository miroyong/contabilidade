...
(function () {
  ...
  // ------------------------------------------------------------ NFC-e (cupom)
  // Lê o QR da NFC-e (via Edge Function do Supabase) e pré-preenche o
  // lançamento com o total/data/loja do cupom.
  function nfcAbrir() {
    state.nfc = null;
    $('nfc-status').hidden = true;
    $('nfc-status').textContent = '';
    $('nfc-result').hidden = true;
    $('nfc-result').innerHTML = '';
    $('nfc-usar').hidden = true;
    $('modal-nfce').showModal();
    setTimeout(function () { $('nfc-url').focus(); }, 50);
  }
  function nfcFechar() {
    if ($('modal-nfce').open) $('modal-nfce').close();
  }
  function nfcBuscar() {
    if (!SB_URL || !SB_KEY) { toast('Configure o Supabase primeiro.'); return; }
    var val = $('nfc-url').value.trim();
    if (!val) { toast('Cole o link do QR ou o código.'); return; }
    var st = $('nfc-status');
    st.hidden = false;
    st.textContent = 'Consultando cupom…';
    $('nfc-result').hidden = true;
    $('nfc-result').innerHTML = '';
    $('nfc-usar').hidden = true;
    fetch(SB_URL + '/functions/v1/nfce', {
      method: 'POST',
      headers: {
        'apikey': SB_KEY,
        'Authorization': 'Bearer ' + SB_KEY,
      ...
      if (!j || !j.ok) {
        st.textContent = 'Erro: ' + ((j && j.erro) || 'não foi possível ler o cupom.');
        return;
      }
      var d = j.dados || {};
      state.nfc = d;
      ...
      if (itens.length) {
        html += '<ul class="nfc-itens">' + itens.slice(0, 12).map(function (i) {
          return '<li>' + esc(i) + '</li>';
        }).join('') + '</ul>';
      }
      $('nfc-result').innerHTML = html;
      $('nfc-result').hidden = false;
      if (d.total) {
        st.hidden = true;
  ...
  // preenche o formulário de lançamento com os dados do cupom
  function nfcUsar() {
    var d = state.nfc;
    if (!d) return;
    nfcFechar();
    abrirModal('novo', 'saida', null); // compras = saída (usuário pode trocar)
    var desc = d.emitente ? ('Compra — ' + d.emitente) : 'Compra NFC-e';
    if (d.chaveCurta) desc += ' (' + d.chaveCurta + ')';
    $('f-descricao').value = desc.slice(0, 120);
    if (d.total) $('f-valor').value = String(d.total);
    if (d.data) $('f-data').value = d.data;
    toast('Cupom preenchido! Confira e salve.');
  }

  // handlers NFC-e
  $('btn-nfc').addEventListener('click', nfcAbrir);
  $('nfc-fechar').addEventListener('click', nfcFechar);
  $('nfc-usar').addEventListener('click', nfcUsar);
  $('form-nfce').addEventListener('submit', function (e) {
    e.preventDefault();
    nfcBuscar();
  });
